...
/**
 * The main class, whose public static void main (psvm) method will be run by the executable jar.
 */
public class App {
    ...
    private static void processSingleFile(Path path) {
        ...
        if (random.getPlain().totalValue() > greedyScore) {
            accumulator.incrementAndGet();
        }

        IntStream.range(1, ITER_NUM).boxed().parallel().forEach(index -> {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            System.out.println("Usage: java -Xmx5g -server -jar target/KP01.jar problemInstances/*/test.in");
            System.exit(1);
        }
        ...
        for (String testFilePath : args) {
            ...
            if (Files.exists(path) && Files.isRegularFile(path))
                ...
                System.err.println("cannot read/find test file: " + testFilePath);
